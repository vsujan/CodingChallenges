...
const evaluatePrefix = (expression: string): number | null => {
  const chars: string[] = expression.split(" ").reverse();
  const refChars: number[] = [];
...
const doOperation = (operator: string, num1: number, num2: number): number => {
  let result: number = 0;
  switch (operator) {
    case "+":
      result = num1 + num2;
